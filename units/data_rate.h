/*
 * data_rate.h - Data rate unit class
 * Ported from WebRTC (api/units/data_rate.h)
 *
...
#ifndef BASE_UNITS_DATA_RATE_H_
#define BASE_UNITS_DATA_RATE_H_
...
// second (bps).
class DataRate final : public base::unit_impl::RelativeUnit<DataRate> {
 public:
  template <typename T>
 ...
 private:
  ...
  // more recognizable.
  friend class base::unit_impl::UnitBase<DataRate>;
...
inline constexpr int64_t Microbits(const DataSize& size) {
  ...
  AVE_DCHECK_LE(size.bytes(), kMaxBeforeConversion)
...
inline constexpr int64_t MillibytePerSec(const DataRate& size) {
  ...
  AVE_DCHECK_LE(size.bps(), kMaxBeforeConversion)

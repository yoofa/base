...
#ifndef BASE_TEST_MOCK_TASK_RUNNER_FACTORY_H_
#define BASE_TEST_MOCK_TASK_RUNNER_FACTORY_H_
...
class MockTaskRunnerBase : public TaskRunnerBase {
 public:
  struct ScheduledTask {
    ...
    ScheduledTask(uint64_t time, std::unique_ptr<Task> t) 
  ...
  void AdvanceTimeUs(uint64_t delta_us) {
    now_us_ += delta_us;
  }
  ...
  void RunDueTasks() {
    ...
    while (!scheduled_tasks_.empty() && scheduled_tasks_.top().due_time_us <= now_us_) {
    ...
    CountDownLatch latch(tasks_to_run.size());
...
class MockTaskRunnerFactory : public TaskRunnerFactory {
 public:
  ...
  std::unique_ptr<TaskRunnerBase, TaskRunnerDeleter> CreateTaskRunner(
    ...
    return std::unique_ptr<TaskRunnerBase, TaskRunnerDeleter>(runner_);
  }
...
#endif  // BASE_TEST_MOCK_TASK_RUNNER_FACTORY_H_

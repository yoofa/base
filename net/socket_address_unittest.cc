...
TEST(SocketAddressTest, TestToSensitiveNameAndAddressString) {
  ...
  SocketAddress hostOnly("google.com", 443);
  EXPECT_EQ("google.com:443", hostOnly.ToSensitiveNameAndAddressString());

  SocketAddress hostAndIpv4("google.com", 80);
  hostAndIpv4.SetResolvedIP(IPAddress(0x01020304));
  EXPECT_EQ("google.com:80 (1.2.3.x:80)",
  ...
  SocketAddress hostAndIpv6("google.com", 5678);
  hostAndIpv6.SetResolvedIP(IPAddress(kTestV6Addr));
  EXPECT_EQ("google.com:5678 (" + kTestV6AddrFullAnonymizedString + ")",

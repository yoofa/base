/*
 * percentile_filter.h - Percentile filter
 *
...
#ifndef BASE_NUMERICS_PERCENTILE_FILTER_H_
#define BASE_NUMERICS_PERCENTILE_FILTER_H_
...
PercentileFilter<T>::PercentileFilter(float percentile)
    : percentile_(percentile),
      ...
      percentile_index_(0) {
  AVE_DCHECK_GE(percentile, 0.0f);
  AVE_DCHECK_LE(percentile, 1.0f);
}
...
void PercentileFilter<T>::UpdatePercentileIterator() {
  ...
  const auto index = static_cast<int64_t>(percentile_ * (set_.size() - 1));
...
#endif  // BASE_NUMERICS_PERCENTILE_FILTER_H_

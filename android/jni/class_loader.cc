...
class ClassLoader {
 public:
  explicit ClassLoader(JNIEnv* env)
      : class_loader_(Java_AveClassLoader_getClassLoader(env)),
        class_loader_class_(env,
                            jni_zero::ScopedJavaLocalRef<jclass>::Adopt(
                                env,
                                env->FindClass("java/lang/ClassLoader"))) {
    AVE_CHECK(!env->ExceptionCheck());
    load_class_method_ =
        env->GetMethodID(class_loader_class_.obj(), "loadClass",
  ...
  jni_zero::ScopedJavaLocalRef<jclass> FindClass(JNIEnv* env,
    ...
    auto j_name = jni_zero::ToJniType<std::string>(env, name);
 ...
 private:
  ...
  jni_zero::ScopedJavaGlobalRef<jclass> class_loader_class_;

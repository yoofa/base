/*
 * rate_statistics.cc - Rate statistics based on counts in time intervals
 * Ported from WebRTC (rtc_base/rate_statistics.cc)
 *
...
void RateStatistics::Update(int64_t count, int64_t now_ms) {
  ...
  // current window size.
  // const int64_t recent_sample_time_margin = current_window_size_ms_ * 3 / 2;
  const auto recent_sample_time_margin = static_cast<int64_t>(
      std::round(static_cast<double>(current_window_size_ms_) * 1.5));
  ...
  if (buckets_.empty() || now_ms != buckets_.back().timestamp) {
    if (!buckets_.empty() && now_ms < buckets_.back().timestamp) {
      AVE_LOG(LS_WARNING) << "Timestamp " << now_ms
...
std::optional<int64_t> RateStatistics::Rate(int64_t now_ms) const {
  // TODO(youfa): use mutable for EraseOld() instead of const_cast.
  ...
  if (num_samples_ == 0 || active_window_size <= 1 ||
      (num_samples_ <= 1 &&
       base::SafeLt(active_window_size, current_window_size_ms_)) ||
      overflow_) {
    return std::nullopt;
  }

  float scale =
      static_cast<float>(scale_) / static_cast<float>(active_window_size);
  float result = static_cast<float>(accumulated_count_) * scale + 0.5f;
  ...
  return base::dchecked_cast<int64_t>(result);
}
...
void RateStatistics::EraseOld(int64_t now_ms) {
  ...
  while (!buckets_.empty() && buckets_.front().timestamp < new_oldest_time) {
    ...
    AVE_DCHECK_GE(accumulated_count_, oldest_bucket.sum);
    AVE_DCHECK_GE(num_samples_, oldest_bucket.num_samples);

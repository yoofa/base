...
// Equivalent to C++20 std::bit_width: returns the number of bits needed
// to represent val (0 returns 0).
constexpr size_t BitWidth(uint32_t val) {
  return val == 0 ? 0u
                  : static_cast<size_t>(CHAR_BIT * sizeof(uint32_t)) -
                        static_cast<size_t>(__builtin_clz(val));
}

constexpr size_t BitWidth(uint64_t val) {
  return val == 0 ? 0u
                  : static_cast<size_t>(CHAR_BIT * sizeof(uint64_t)) -
                        static_cast<size_t>(__builtin_clzll(val));
}
...
uint8_t WritePartialByte(uint8_t source,
  ...
  AVE_DCHECK(target_bit_offset < 8);
  AVE_DCHECK(source_bit_count < 9);
  AVE_DCHECK(source_bit_count <= (8 - target_bit_offset));
...
BitBufferWriter::BitBufferWriter(uint8_t* bytes, size_t byte_count)
    : writable_bytes_(bytes),
      ...
      byte_offset_(),
      bit_offset_() {
  AVE_DCHECK(static_cast<uint64_t>(byte_count_) <=
...
void BitBufferWriter::GetCurrentOffset(size_t* out_byte_offset,
  ...
  AVE_CHECK(out_byte_offset != nullptr);
  AVE_CHECK(out_bit_offset != nullptr);
...
bool BitBufferWriter::WriteExponentialGolomb(uint32_t val) {
  ...
  return WriteBits(val_to_encode, BitWidth(val_to_encode) * 2 - 1);
}
...
bool BitBufferWriter::WriteLeb128(uint64_t val) {
  bool success = true;
  do {
    auto byte = static_cast<uint8_t>(val & 0x7f);
    val >>= 7;
    if (val > 0) {
      byte |= 0x80;
    }
...
bool BitBufferWriter::WriteString(std::string_view data) {
  bool success = true;
  for (char c : data) {

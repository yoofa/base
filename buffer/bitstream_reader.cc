...
constexpr int BitWidth(uint32_t val) {
  return val == 0 ? 0
                  : CHAR_BIT * static_cast<int>(sizeof(uint32_t)) -
                        __builtin_clz(val);
}
...
uint64_t BitstreamReader::ReadBits(int bits) {
  AVE_DCHECK_GE(bits, 0);
  AVE_DCHECK_LE(bits, 64);
...
uint32_t BitstreamReader::ReadNonSymmetric(uint32_t num_values) {
  AVE_DCHECK_GT(num_values, 0u);
  AVE_DCHECK_LE(num_values, uint32_t{1} << 31);

  int width = BitWidth(num_values);
  ...
  return (static_cast<uint32_t>(val) << 1) + ReadBit() - num_min_bits_values;
}
...
int BitstreamReader::ReadSignedExponentialGolomb() {
  ...
  return static_cast<int>((unsigned_val + 1) / 2);
}
...
std::string BitstreamReader::ReadString(int num_bytes) {
  ...
  for (int i = 0; i < num_bytes; ++i) {
    res += static_cast<char>(Read<uint8_t>());
  }
